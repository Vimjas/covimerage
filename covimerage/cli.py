...
@click.option(
    ...
    show_default=True,
    help=(
        "Set logging level explicitly (overrides -v/-q).  "
...
@click.option(
    ...
    help=(
        "File name for the PROFILE_FILE file.  " "By default a temporary file is used."
    ),
)
@click.option(
    "--data-file",
    ...
    type=click.File("w"),
    help=(
        "DATA_FILE to write into.  " u"[default:\xa0%s]" % DEFAULT_COVERAGE_DATA_FILE
    ),
)
@click.option(
    "--append",
        ...
        if report_options:
            ...
            try:
                ...
                raise click.exceptions.UsageError(
                    "Failed to parse --report-options: %s" % str(exc), ctx=ctx
                )
        ...
        if not os.path.exists(profile_file):
            if not exit_code:
                raise CustomClickException(
                    "The profile file (%s) has not been created." % (profile_file)
                )
            ...
            if report:
                ...
                ctx.invoke(report_cmd, report_file=report_file, **report_opts)

    if exit_code != 0:
        raise CustomClickException(
            "Command exited non-zero: %d." % exit_code, exit_code=exit_code
        )
...
@main.command()
@click.argument(
    "profile_file", type=click.File("r"), required=False, nargs=-1, is_eager=True
)
@click.option(
    "--data-file",
    required=False,
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help=("Show line numbers of statements in each file that was not executed."),
)
@click.option(
    "--include",
    required=False,
...
@click.option(
    "--skip-covered", is_flag=True, default=False, help="Skip files with 100% coverage."
)
@click.option(
    "--source",
    type=click.types.Path(exists=True),
...
def report(
    ...
    CoverageWrapper(data=data, data_file=data_file, config_file=config_file).report(
...
def xml(ctx, data_file, include, omit, ignore_errors):
    ...
    config_file = ctx.obj.get("rcfile") if ctx.obj else None
    CoverageWrapper(data_file=data_file, config_file=config_file).reportxml(
