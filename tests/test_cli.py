...
def test_cli_run_with_args_fd(capfd):
    ...
    assert (
        lines
        == [
            "Running cmd: echo -- --no-profile %sMARKER --cmd 'profile start /doesnotexist' --cmd 'profile! file ./*' (in {})".format(
                os.getcwd()
            ),
...
def test_cli_run_args(runner, mocker, devnull, tmpdir):
    ...
    m.return_value = 1
    result = runner.invoke(cli.run, ["--no-wrap-profile", "printf", "--headless"])
    ...
    m.return_value = 0
    with tmpdir.as_cwd() as old_dir:
        profile_file = str(old_dir.join("tests/fixtures/conditional_function.profile"))
        result = runner.invoke(
            cli.run,
            [
                "--no-wrap-profile",
...
def test_cli_call_verbosity_fd(capfd):
    ...
    assert (
        err.splitlines()
        == [
            "Not writing coverage file: no data to report!", "Error: No data to report."
        ]
    )
...
def test_cli_writecoverage_datafile(runner):
    ...
    assert result.output == "\n".join(["Writing coverage file %s." % f, ""])
    assert result.exit_code == 0
...
def test_cli_writecoverage_source(runner):
    ...
    cov = CoverageWrapper(data_file=f)
    assert (
        cov.lines[os.path.abspath("tests/test_plugin/conditional_function.vim")]
        == [3, 8, 9, 11, 13, 14, 15, 17, 23]
    )
    assert (
        cov.lines[os.path.abspath("tests/test_plugin/autoload/test_plugin.vim")] == []
    )


def test_coverage_plugin_for_annotate_merged_conditionals(runner, capfd, tmpdir):
    ...
    assert result.output == "\n".join(["Writing coverage file %s." % tmpfile, ""])
    assert result.exit_code == 0
...
def test_report_profile_or_data_file(runner, tmpdir):
    ...
    result = runner.invoke(cli.main, ["report", "--data-file", "/does/not/exist"])
    assert (
        result.output.splitlines()[-1]
    ...
    result = runner.invoke(cli.main, ["report", "--data-file", os.devnull])
...
def test_report_rcfile_and_include(tmpdir, runner):
    ...
    with tmpdir.as_cwd() as old_cwd:
        profile_file = str(old_cwd.join("tests/fixtures/conditional_function.profile"))
        tmpdir.join(profiled_file).write(profiled_file_content, ensure=True)
        ...
        result = runner.invoke(
            cli.main, ["--rcfile", coveragerc, "report", profile_file]
        )
        assert (
            result.output.splitlines()
            == [
...
def test_cli_xml(runner, tmpdir):
    ...
    with tmpdir.as_cwd() as old_cwd:
        ...
        with open("coverage.xml") as f:
            xml = f.read()
        assert (
            'filename="%s/tests/test_plugin/merged_conditionals.vim' % (old_cwd) in xml
        )
        ...
        result = runner.invoke(
            cli.main, ["--rcfile", coveragerc, "xml", "--data-file", ".coverage"]
        )
        assert result.output == ""
        assert result.exit_code == 0
...
def test_run_handles_exit_code_from_python_fd(capfd):
    ret = call(
        [
            ...
            "python",
            "-c",
            'print("output"); import sys; sys.exit(42)',
        ]
    )
    out, err = capfd.readouterr()
...
def test_run_handles_exit_code_from_python_pty_fd(capfd):
    ret = call(
        [
            ...
            "python",
            "-c",
            "import pty; pty.spawn(['/bin/sh', '-c', " "'printf output; exit 42'])",
        ]
    )
    out, err = capfd.readouterr()
    assert "Error: The profile file (/not/used) has not been created." in err.splitlines()
    assert out == "output"
    assert ret == 1
    ...
    with tmpdir.as_cwd() as old_dir:
        ...
        assert (
            ...
            == [
                "Running cmd: printf -- --headless (in %s)" % str(tmpdir),
                "Parsing profile file %s." % profile_file,
                'Error: Coverage could not read data_file: %r (CoverageException("Doesn\'t seem to be a coverage.py data file",))'
                % data_file,
            ]
        )
        assert result.exit_code == 1
...
def test_run_append_with_data(runner, tmpdir):
    ...
    with tmpdir.as_cwd() as old_dir:
        ...
        tmpdir.join(profiled_file).write(profiled_file_content, ensure=True)
        profile_file = str(old_dir.join("tests/fixtures/merged_conditionals-0.profile"))
        tmpdir.join(profiled_file).write(profiled_file_content, ensure=True)
        result = runner.invoke(
            cli.run,
            [
