...
def test_build_vim_profile_args(devnull, tmpdir):
    ...
    assert (
        F(fname, [fname])
        == ["--cmd", "profile start %s" % fname, "--cmd", "profile! file %s" % fname]
    )
    assert (
        F(fname, [dname])
        == ["--cmd", "profile start %s" % fname, "--cmd", "profile! file %s/*" % dname]
    )
    assert (
        F(fname, [fname, dname])
        == [
            "--cmd",
            "profile start %s" % fname,
            "--cmd",
            "profile! file %s" % fname,
            "--cmd",
            "profile! file %s/*" % dname,
        ]
    )
