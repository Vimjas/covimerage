...
def test_filereporter():
    ...
    with pytest.raises(coverage.misc.NoSource) as excinfo:
        f.lines()
    assert (
        excinfo.value.args == ("[Errno 2] No such file or directory: '/doesnotexist'",)
    )
...
def test_filereporter_source_handles_latin1(tmpdir):
    ...
    with tmpdir.as_cwd():
        ...
        assert FileReporter("iso.txt").source().encode("utf-8") == b"Hellstr\xc3\xb6m"
        assert FileReporter("iso.txt").source().encode("utf-8") == b"Hellstr\xc3\xb6m"
...
def test_coveragedata(coverage_fileobj):
    ...
    assert excinfo.value.args == ("data and data_file are mutually exclusive.",)
    ...
    assert (
        excinfo.value.args == ("data needs to be of type coverage.data.CoverageData",)
    )

    with pytest.raises(CoverageWrapperException) as excinfo:
        CoverageData(data_file="/does/not/exist")
    assert excinfo.value.args == ("Coverage could not read data_file: /does/not/exist",)
    ...
    assert (
        repr(e)
        == "CoverageWrapperException(message=%r, orig_exc=%r)" % (e.message, e.orig_exc)
    )
    ...
    assert (
        cov_data.lines
        == {"/test_plugin/conditional_function.vim": [3, 8, 9, 11, 13, 14, 15, 17, 23]}
    )
...
def test_coveragewrapper(coverage_fileobj, devnull):
    ...
    assert excinfo.value.args == ("Coverage could not read data_file: /dev/null",)
