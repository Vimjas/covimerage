...
def test_profile_fname_or_fobj(caplog, devnull):
    ...
    assert (
        str(excinfo.value) == "[Errno 2] No such file or directory: '/does/not/exist'"
    )
...
def test_line():
    ...
    assert (
        repr(Line(line))
        == "Line(line=%r, count=None, total_time=None, self_time=None)" % (line)
    )
...
def test_profile_parse():
    ...
    assert (
        ...
        == {
            1: Line(
                line="function! test_plugin#func1(a) abort",
                count=None,
                total_time=None,
                self_time=None,
            ),
            2: Line(
                line="  echom 'func1'", count=1, total_time=None, self_time=3.5e-05
            ),
            3: Line(line="endfunction", count=None, total_time=None, self_time=None),
            4: Line(line="", count=None, total_time=None, self_time=None),
            5: Line(
                line="function! test_plugin#func2(a) abort",
                count=1,
                total_time=None,
                self_time=5e-06,
            ),
            6: Line(line="  echom 'func2'", count=0, total_time=None, self_time=None),
            7: Line(line="endfunction", count=None, total_time=None, self_time=None),
...
def test_profile_parse_dict_function():
    ...
    assert (
        [l.line for l in s.lines.values()]
        == [
...
def test_profile_continued_lines():
    ...
    assert (
        [(l.count, l.line) for l in s.lines.values()]
        == [(N, "echom 1"), (1, "echom 2"), (N, "      \\ 3"), (1, "echom 4")]
    )
...
def test_mergedprofiles_fixes_line_count():
    ...
    N = None
    assert (
        [(l.count, l.line) for l in script.lines.values()]
        == [(N, "echom 1"), (1, "echom 2"), (N, "      \\ 3"), (1, "echom 4")]
    )

    m = MergedProfiles([p])
    assert (
        [(l.count, l.line) for l in m.lines[script.path].values()]
        == [(1, "echom 1"), (1, "echom 2"), (N, "      \\ 3"), (1, "echom 4")]
    )
...
def test_merged_profiles_write_coveragepy_data_handles_fname_and_fobj(mocker, caplog):
    from covimerage import MergedProfiles
    ...
    assert (
        msgs
        == ["Writing coverage file .coverage.", "Writing coverage file %s." % fileobj]
    )
...
def test_function_in_function_with_ref(caplog):
    ...
    assert (
        [(l.count, l.line) for l in s.lines.values() if not l.line.startswith('"')]
        == [
            (None, ""),
            (1, "let g:refs = []"),
            (None, ""),
